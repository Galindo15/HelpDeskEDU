...
def watchers(self, ticket_id: int) -> list[User]:
    """
    Devuelve los observadores del ticket (solicitante y técnico asignado si existe).
    Deduplica si ambos son la misma persona.
    """
    ticket = self.require(ticket_id)
    ...
    if ticket.assignee_id is not None and ticket.assignee_id not in user_ids:
        user_ids.append(ticket.assignee_id)
        
    # Obtiene los usuarios usando el servicio de usuarios, sin acceder a repositorios directamente
    return [self._users.require(uid) for uid in user_ids]

def assign(self, ticket_id: int, technician_id: int) -> Ticket:
    ticket = self.require(ticket_id)
    technician = self._users.require(technician_id)

    # Revisa si ya está asignado antes de modificar estado o disparar notificaciones
    if ticket.assignee_id == technician_id:
        raise DuplicateAssignmentError(
            f"El ticket {ticket_id} ya se encuentra asignado al técnico {technician_id}."
        )
    ...
    if self._notifier:
        self._notifier.notify("ticket_assigned", {"ticket_id": ticket.id, "assignee_id": technician_id})
